...
**基本理念：「時間管理」ツールではなく「時間拡張」ツール。アシスタントではなく同僚。**
...
本プロジェクトは3エージェント協業開発モデルを採用しています。詳細は[マルチエージェント開発ガイド](docs/08_多Agent协作开发方案.md)（中国語）を参照してください。

```bash
# 異なるエージェントIDで Claude Code を起動
source scripts/agent-zhangsan.sh        # ユーザーインタラクション層
source scripts/agent-lisi.sh            # コアエンジン層
source scripts/agent-zhangchonglian.sh  # インフラストラクチャ
```
